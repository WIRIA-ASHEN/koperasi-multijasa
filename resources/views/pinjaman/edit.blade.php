...
<div class="row">
  ...
  <main class="col-md-9  col-lg-10 px-md-4">
    <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
      <h1 class="h2">Edit Pinjaman</h1>
    </div>
    
    <form action="/pinjaman-update/{{ $pinjamans->id }}" method="post">
      {{ csrf_field() }}
      <div class="mb-3">
        <label for="id" class="form-label ">ID Anggota</label>
        <select class="form-select" id="id_anggota" name="id_anggota">
          <option value="{{ $pinjamans->anggotas->id }}" selected>{{ $pinjamans->anggotas->nama }} - {{ $pinjamans->anggotas->nik }}</option>
      ...
      <div class="mb-3">
        ...
        <input type="text" class="form-control" id="nama" name="nama" value="{{$pinjamans->anggotas->nama}}">
      </div>
      <div class="mb-3">
        <label for="tgl_pinjaman" class="form-label">Tanggal Pinjaman</label>
        <input type="date" class="form-control" name="tgl_pinjaman" id="tgl_pinjaman" required value="{{$pinjamans->tgl_pinjaman}}">
      </div>
      <div class="mb-3">
        <label for="besar_pinjaman">Jumlah</label>
        <input type="number" class="form-control" id="besar_pinjaman" name="besar_pinjaman" autocomplete="off" readonly value="{{$pinjamans->besar_pinjaman}}" disabled>
      </div>
      <div class="mb-3">
        <label for="bagi_hasil">Bagi Hasil (%)</label>
        <input type="text" class="form-control" id="bagi_hasil" name="bagi_hasil" value="{{ $pengaturans->jasa_pinjaman }}" disabled readonly>
      </div>
      <div class="mb-3">
        <label for="jangka_waktu">Jangka Waktu (bulan)</label>
        <input type="text" class="form-control" id="jangka_waktu" name="jangka_waktu" value="{{ $pinjamans->jangka_waktu }}" autocomplete="off" readonly disabled>
      </div>
      <div class="mb-3">
        <label for="bayar_perbulan">Bayar Perbulan</label>
        <input type="text" class="form-control" id="bayar_perbulan" name="bayar_perbulan" value="{{$pinjamans->bayar_perbulan}}" readonly disabled>
      </div>
      <div class="mb-3">
        <label for="keterangan">Keterangan</label>
        <input type="text" class="form-control" id="keterangan" name="keterangan" value="{{$pinjamans->keterangan}}" >
      </div>
