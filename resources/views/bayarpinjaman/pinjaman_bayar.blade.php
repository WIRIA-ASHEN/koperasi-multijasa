  ...
  <div class="row">
    ...
    <main class="col-md-9  col-lg-10 px-md-4">
      <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-2 border-bottom">
        <h1 class="h2">Angsuran</h1>
      </div>

      <form action="/bayar-pinjaman" method="GET" class="mb-3">
        ...
        <table class="table table-hover table-bordered">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>{{ $data->pinjamans->anggotas->nik }}</td>
                    <td>{{ \Carbon\Carbon::parse($data->jatuh_tempo)->locale('id')->isoFormat('D MMMM YYYY') }}</td>
                    <td>
                      @if (!empty($data->tanggal_bayar))
                      {{ \Carbon\Carbon::parse($data->tanggal_bayar)->locale('id')->isoFormat('D MMMM YYYY') }}
                      @else
                      {{ $data->tanggal_bayar }}
                      @endif
                    </td>
                    ...
                    <td>{{ $data->status }}</td>
                    <td>
                      <a href="/bayar-pinjaman/bayar/{{ $data->id }}" class="btn btn-sm btn-warning">Bayar</a>
                      @if (auth()->user()->role == 'ketua')
                      <a href="/bayar-pinjaman/hapus/{{ $data->id }}" onclick="return confirm('Hapus Data yang sudah bayar?');" class="btn btn-sm btn-primary"><i class="bi bi-check2-square"></i></a>
                      @endif
                    </td>
                </tr>
                @endforeach
    ...
    $(document).ready(function () {
      var rowsShown = 10; // Number of rows to show per page
