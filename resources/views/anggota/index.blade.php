...
<div class="container-fluid">
  <div class="row">
    ...
    <main class="col-md-9  col-lg-10 px-md-4">
      ...
      <a href="/tambah-anggota" class="btn btn-success mb-2">Tambah anggota</a>
      ...
      <table class="table">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>{{ $anggota->id }}</td>
                <td>{{ $anggota->nik }}</td>
                <td>{{ \Carbon\Carbon::parse($anggota->tgl_lahir)->locale('id')->isoFormat('D MMMM YYYY') }}</td>
                <td>{{ $anggota->tmpt_lahir }}</td>
                <td>{{ $anggota->alamat }}</td>
                <td>{{ $anggota->no_telp }}</td>
                <td>{{ $anggota->user->username }}</td>
                <td>
                  <a href="/anggota/edit/{{ $anggota->id }}" style="color: black;" class="btn btn-sm btn-warning"><i class="bi bi-pencil-square"></i></a> 
                  @if (auth()->user()->role == 'ketua')
                  |
                  <a href="/anggota-hapus/{{ $anggota->id }}" style="color: black;" onclick="return confirm('Anda yakin ingin menghapus?');" class="btn btn-sm btn-danger"><i class="bi bi-trash" ></i></a>
                  @endif
                </td>
            </tr>
            @endforeach
